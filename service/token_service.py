...
async def get_token_info(bearer_token):
    ...
    token_info = await TokensEntity.get_by_key(token_without_sk)
...
def do_add_token(data: AddTokenPo):
    ...
    return TokensEntity.create_token(key, data.name, data.status, data.user_id, data.expired_time)


def do_del_token(token_id):
    return TokensEntity.delete_token(token_id)


def do_change_token_status(token_id, status):
    return TokensEntity.change_status(token_id, status)


def do_update_token(token_id, data: AddTokenPo):
    try:
        token = TokensEntity.get_token_by_id(token_id)
        token.name = data.name
        token.expired_time = data.expired_time
        token.save()
        return token
    except DoesNotExist:
        raise Exception("令牌不存在")
